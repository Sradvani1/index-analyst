...
def build_state_prompt(
    *,
    ...
    parts.append(
        ...
        "4. Build `decision_matrix_rows` with all 18 framework rows using their exact labels.\n\n"
        "Numeric fields are re-verified by the engine, so do not tune them:\n"
        f"- Emit the `mc_*` fields and `spx_close` as schema-valid copies from analysis_context.\n"
        f"- For these precompute-owned rows, put a brief '(engine-filled)' placeholder in "
        f"current_reading/signal: {owned_rows}.\n\n"
        "`signals_*` contract (schema is strict — extra keys fail validation):\n"
        "- Use only keys with the `signals_` prefix at `emit_daily_state`'s top level; "
        "no `*_detail`, `*_note`, or extra `*_zone` fields.\n"
        "- `signals_fear_greed` + `signals_fear_greed_zone` is the only score+label pair.\n"
        "- `signals_vix_regime`: one string with zone and level/MA context; no "
        "`signals_vix_regime_detail` or `signals_vix`.\n"
...
def build_report_prompt(
    *,
    ...
    parts.append(
        ...
        "conflict checklist as background inputs; do not quote filenames or framework-rule labels "
        "in published prose.\n\n"
        "Do not regenerate numerics in prose where Python injects a facts block — interpret the "
        "read-only snippets instead. Use bullet lists for key levels, session triggers, or when "
        "multiple tensions need separating; short prose is fine for a single clear tension.\n\n"
        ...
        "and how today's validated posture resolves the tension. On zero-divergence days, cover "
        "primary_tension and confirming evidence explicitly."
        f"{mixed_note}"
        f"{pass2_task_extra}"
    )
    return PromptBundle(system_role=system_role, framework=framework, body="\n\n".join(parts))
